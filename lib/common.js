...
function overrideRequests(newRequest) {
  ...
  ;['http', 'https'].forEach(function(proto) {
    ...
    debug('overridenrequest was');
    debug(overriddenRequest.toString());
...
function restoreOverriddenRequests() {
  ...
  debug(requestOverrides);
  Object.entries(requestOverrides).forEach(
    ([proto, { module, request, get }]) => {
      debug('- restoring request for', proto)
      debug('- module', module.request.toString())
      module.request = request
      module.get = get
      debug('- restored request for', proto)
      debug('- original request', request.toString())
    }
  )
  debug('setting requestoverrides to empty');
  requestOverrides = {}
}
